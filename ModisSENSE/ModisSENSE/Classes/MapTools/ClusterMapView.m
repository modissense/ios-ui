...
- (id)initWithFrame:(CGRect)frame latitude:(CLLocationDegrees)lat longitude:(CLLocationDegrees)lng callout:(NSString*)calloutStr calloutSub:(NSString*)calloutSubStr accessory:(BOOL)accessory pinType:(PinType)type draggable:(BOOL)draggable selectTarget:(id)target selectAction:(SEL)action {
	NSDictionary *dict = [NSDictionary dictionaryWithObjectsAndKeys:
												...
												nil];
	return [self initWithFrame:(CGRect)frame locations:[NSArray arrayWithObject:dict] polyline:nil draggable:draggable selectTarget:target selectAction:action];
}

// initializer method for multiple fixed pins
- (id)initWithFrame:(CGRect)frame locations:(NSArray*)locations {
	return [self initWithFrame:(CGRect)frame locations:locations polyline:nil draggable:NO selectTarget:nil selectAction:nil];
}
...
- (id)initWithFrame:(CGRect)frame locations:(NSArray*)locations polyline:(NSArray*)polyline {
	return [self initWithFrame:(CGRect)frame locations:locations polyline:polyline draggable:NO selectTarget:nil selectAction:nil];
}
...
- (void)show: (BOOL)showPins {
    ...
    if (showPins) {
        ...
        for (NSInteger i=0; i<[iLocations count]; i++) {
            ...
            annotation.title = calloutStr;
            annotation.subtitle = calloutSubStr;
            CLLocationCoordinate2D newCoord = {[(NSNumber*)[dict objectForKey:MAP_LATITUDE_KEY] doubleValue], [(NSNumber*)[dict objectForKey:MAP_LONGITUDE_KEY] doubleValue]};
...
//    NSRange theRange;
//    theRange.location = 0;
//    theRange.length = [iLocations count] -1;
//    iLocations = [iLocations subarrayWithRange:theRange];
}
...
- (MKAnnotationView *)mapView:(MKMapView *)mapView viewForAnnotation:(id <MKAnnotation>)annotation {
    ...
    NSInteger pinType = [(NSNumber*)[(NSDictionary*)[iLocations objectAtIndex:((REVClusterPin *)annotation).index] objectForKey:MAP_PIN_TYPE_KEY] intValue];
       NSString *pinIcon = [(NSDictionary*)[iLocations objectAtIndex:((REVClusterPin *)annotation).index] objectForKey:MAP_PIN_ICON_KEY];
    BOOL accessory = [(NSNumber*)[(NSDictionary*)[iLocations objectAtIndex:((REVClusterPin *)annotation).index] objectForKey:MAP_ACCESSORY_KEY] boolValue];
    NSString *accessoryIcon = [(NSDictionary*)[iLocations objectAtIndex:((REVClusterPin *)annotation).index] objectForKey:MAP_ACCESSORY_ICON_KEY];
    NSString *leftIcon = [(NSDictionary*)[iLocations objectAtIndex:((REVClusterPin *)annotation).index] objectForKey:MAP_LEFT_ICON_KEY];
...
- (void)mapView:(MKMapView *)mapView didAddAnnotationViews:(NSArray *)views {
    if( [self.delegate respondsToSelector:@selector(mapView:didAddAnnotationViews:)] ) {
        return [self.delegate mapView:mapView didAddAnnotationViews:views];
    }
...
//				UIImage *thumb = [Tk captureView:self rect:r];
...
- (void)mapView:(MKMapView *)mapView annotationView:(MKAnnotationView *)view calloutAccessoryControlTapped:(UIControl *)control {
	...
	if (self.gMapDelegate && [self.gMapDelegate respondsToSelector:@selector(accessoryTappedForAnnotation:)])
		[self.gMapDelegate accessoryTappedForAnnotation:((REVClusterPin*)view.annotation).tag];
}
...
- (void)mapView:(MKMapView *)mapView selectedAnnotation:(id <MKAnnotation>)annotation {
	...
	if (self.gMapDelegate && [self.gMapDelegate respondsToSelector:@selector(selectedAnnotation:)])
		[self.gMapDelegate selectedAnnotation:((REVClusterPin*)annotation).tag];
}
- (void)mapView:(MKMapView *)mapView didSelectAnnotationView:(MKAnnotationView *)view {
    [super mapView:mapView didSelectAnnotationView:view];
    
    if (self.selectionDelegate && [self.selectionDelegate respondsToSelector:@selector(didSelectPin:onMap:)]) {
        [self.selectionDelegate didSelectPin:view onMap:mapView];
    }
}
