...
- (void) mapView:(MKMapView *)mapView regionDidChangeAnimated:(BOOL)animated
{
    ...
    {
        NSEnumerator *e = [self.annotations objectEnumerator];
        MKAnnotationView *object;
        NSMutableArray *annotations = [[NSMutableArray alloc] init];
        while (object = [e nextObject]) {
            NSLog(@"Annotation class: %@", [object class]);
            if (![[object class] isSubclassOfClass:[MKUserLocation class]]) {
                [annotations addObject:object];
            }
        }
        
        [super removeAnnotations:annotations];
        super.showsUserLocation = self.showsUserLocation;
    }
